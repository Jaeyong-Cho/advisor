---
name: principle-record-and-resolve-friction
...
3. **Decide and act on each item.** For code, first understand it through [Understand Through Abstraction](../principle-understand-through-abstraction/SKILL.md). Confirm the real problem and cause, then improve the needed code, design, tool, or verification. Leave evidence when no action is needed.
4. **Record the result and evidence.** Record what you did and how you checked it, then mark it complete. Verify changes through [Closed Working Loop](../principle-closed-working-loop/SKILL.md). Add newly discovered friction to the same list.

When the same friction or correction repeats, apply [Encode Lessons in Structure](../principle-encode-lessons-in-structure/SKILL.md) to prevent recurrence.
