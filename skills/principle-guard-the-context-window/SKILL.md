...
5. **Treat output as a cost.** Configure tools and checks to show only necessary output. Turn recurring verification into a rerunnable tool with [Build the Lever](../principle-build-the-lever/SKILL.md). When several investigation paths are needed, use only their conclusions and evidence for the next decision.
