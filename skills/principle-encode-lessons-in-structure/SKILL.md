...
5. **Check the current and next case.** Resolve the current problem and verify that the guard prevents the error for the same input or change. Apply [Build the Lever](../principle-build-the-lever/SKILL.md) when a tool must perform or prove recurring work.
