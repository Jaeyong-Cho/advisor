...
2. **Trace from symptom to cause.** Follow [Understand Through Abstraction](../understand-through-abstraction/SKILL.md): begin at the entry function, inspect responsibilities and contracts, and read only necessary functions line by line. Ask why each state or result occurs.
...
7. **Verify through the reproduction path.** Use [Closed Working Loop](../closed-working-loop/SKILL.md) to check the same condition that produced the pre-fix symptom. If a clear, cheap local test path exists, run [TDD Bug Fix](../tdd/SKILL.md) before changing production code.
