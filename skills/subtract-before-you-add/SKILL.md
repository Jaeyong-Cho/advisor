...
1. **Confirm current purpose and use.** Use [Define Goal](../define-goal/SKILL.md) to state required behavior and contracts to preserve. Do not treat speculative future use or support as a current requirement.
...
3. **Remove first and verify.** Remove a small candidate and use [Closed Working Loop](../closed-working-loop/SKILL.md) to check preserved behavior. Use the simpler structure as the base for the next change.
