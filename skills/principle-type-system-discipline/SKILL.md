...
1. **Make invalid states unrepresentable.** Do not model mutually exclusive fields as a collection of optional values. Use explicit state kinds and values required by each state. Define states and transitions with [Model the Domain](../principle-model-the-domain/SKILL.md).
...
3. **Do not trust external data before parsing.** Validate JSON, CLI arguments, configuration, environment variables, database rows, and network responses at boundaries, then convert them into typed domain values. Follow [Boundary Discipline](../principle-boundary-discipline/SKILL.md).
...
6. **Derive types from the authoritative definition.** Do not hand-copy a schema, protocol, or database migration that already owns the shape. Prevent repeated drift structurally with [Encode Lessons in Structure](../principle-encode-lessons-in-structure/SKILL.md).
