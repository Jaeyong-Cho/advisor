...
4. **Put it in the working loop.** Connect the tool's output and verification criteria to [Closed Working Loop](../closed-working-loop/SKILL.md). The tool itself is subject to review and reruns.
5. **Keep the tool with the work.** Save the tool and its usage when a future task or review can reuse it. Use [Laziness Protocol](../laziness-protocol/SKILL.md) to limit the tool to the smallest scope that performs or proves the work.
