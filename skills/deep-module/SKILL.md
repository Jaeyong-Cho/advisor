...
1. **Define the necessary responsibility.** When redesigning in Chill Mode, use [First Principle Redesign](../first-principle-redesign/SKILL.md) to establish the purpose, required behavior, and constraints before choosing a boundary.
...
State externally observable failure conditions and side effects in the contract. [Understand Through Abstraction](../understand-through-abstraction/SKILL.md) explains a subject from its external contract inward; Deep Module designs a subject that callers can use through that contract.

## Stop When

Representative callers can complete necessary work from the contract without reading implementation or coordinating internal state, and related implementation changes remain inside the module. Verify preserved behavior through [Closed Working Loop](../closed-working-loop/SKILL.md) after structural changes.
