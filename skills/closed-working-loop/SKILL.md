...
1. **Name the difference to verify.** State what must change from the current state. Apply [Define Goal](../define-goal/SKILL.md) first when the goal is unclear.
...
Break refactoring into small changes whose behavior can be checked after each step. If the same change or check repeats, apply [Build the Lever](../build-the-lever/SKILL.md) and include a rerunnable tool in the loop.
